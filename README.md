...
| `NintendoSwitchControlLibrary-1.3.1/` | 第三方库（上游 MIT），需放入 Arduino `libraries` |
...
| `NintendoSwitchControlLibrary-1.3.1/` | Third-party library (MIT); must live under Arduino `libraries` |
...
## 推送到 GitHub（首次）/ First push to GitHub

**中文** 本仓库已在本地完成 **`main` 分支首次提交**（`git init` 已做过，无需重复）。接下来只需：

1. 在 [github.com/new](https://github.com/new) 新建一个**空**仓库（**不要**勾选 “Add a README”），记下仓库地址。  
2. 在本机项目目录执行（将 URL 换成你的；HTTPS 或 SSH 均可）：
...
若提示 `remote origin already exists`，改执行：`git remote set-url origin <新地址>` 再 `git push -u origin main`。

**和 SVN 的粗略对应**（方便记）：`git add` + `git commit`＝在本地做一版快照；`git push` 才相当于把本机变更送到服务器；`git pull` 接近 `svn update`。日常交给我在对话里说「推一下」即可。

**EN** The repo already has an initial commit on **`main`**. Create an **empty** GitHub repository (no README from the template), then:
...
If `origin` already exists, use `git remote set-url origin <url>` then push again.
