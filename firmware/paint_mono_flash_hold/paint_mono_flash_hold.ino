/**
 * 单色自动落笔（paint_mono_flash 副本 + 混合拖线）
 *
 * 在 [`paint_mono_flash`](../paint_mono_flash) 同款刀路（蛇形、斜向空跑、行裁剪）基础上：
 * - 水平段长度 **len < PAINT_HOLD_LINE_MIN_LEN（默认 16）**：**A + pushHat** 离散连点（与原版一致，避开死区/初加速）。
 * - **len ≥ 16**：**长按**方向，`delay` 由巡航式 **T = HOLD_LINE_SLOPE_NUM/DEN * len + HOLD_LINE_MS_OFFSET**（默认 4.11*L+370 ms）。
 *
 * 原版 `paint_mono_flash` 目录不修改；掩码请同步本目录 `draw_data.h`。
 *
...
/** len ≥ 此值时用长按拖线；即「长度 > 15 格」走 hold */
#ifndef PAINT_HOLD_LINE_MIN_LEN
#define PAINT_HOLD_LINE_MIN_LEN 16u
#endif

/** T_ms ≈ (HOLD_LINE_SLOPE_NUM * len) / HOLD_LINE_SLOPE_DEN + HOLD_LINE_MS_OFFSET；默认 4.11*L+370 */
#ifndef HOLD_LINE_SLOPE_NUM
#define HOLD_LINE_SLOPE_NUM 411u
#endif
#ifndef HOLD_LINE_SLOPE_DEN
#define HOLD_LINE_SLOPE_DEN 100u
#endif
#ifndef HOLD_LINE_MS_OFFSET
#define HOLD_LINE_MS_OFFSET 370ul
#endif
#ifndef HOLD_LINE_MS_MAX
#define HOLD_LINE_MS_MAX 1200ul
#endif

static uint8_t g_row = HOME_ROW0;
static uint8_t g_col = HOME_COL0;

static unsigned long holdLineMsFromLen(unsigned len) {
  return (unsigned long)HOLD_LINE_SLOPE_NUM * (unsigned long)len / (unsigned long)HOLD_LINE_SLOPE_DEN +
         (unsigned long)HOLD_LINE_MS_OFFSET;
}
...
static void paintHorizontalRun(uint8_t r, uint8_t c_lo, uint8_t c_hi, bool ltr) {
  ...
  if (len < (unsigned)PAINT_HOLD_LINE_MIN_LEN) {
    ...
    unsigned long ms = holdLineMsFromLen(len);
    if (ms > HOLD_LINE_MS_MAX) {
      ms = HOLD_LINE_MS_MAX;
    }
    SwitchControlLibrary().pressButton(Button::A);
    SwitchControlLibrary().sendReport();
    delay(LINE_A_PRIME_MS);
    SwitchControlLibrary().pressHatButton(hat_paint);
    SwitchControlLibrary().sendReport();
    delay(ms);
